...
def equation(t, r):
    ...
    dvx1dt -= (k * v_rel[0] + (dipole_interaction(dist) * 1.26) / (10 ** 4.5))
    dvy1dt -= (k * v_rel[1] + (dipole_interaction(dist) * 1.26) / (10 ** 4.5))
    dvx2dt += (k * v_rel[0] + (dipole_interaction(dist) * 1.26) / (10 ** 4.5))
    dvy2dt += (k * v_rel[1] + (dipole_interaction(dist) * 1.26) / (10 ** 4.5))
