...
def equation(t, r):
    ...
    dvx1dt -= dipole_interaction(dist) / 10e3
    dvy1dt -= dipole_interaction(dist) / 10e3
    dvx2dt += dipole_interaction(dist) / 10e3
    dvy2dt += dipole_interaction(dist) / 10e3
...
def update(frame):
    ...
    r_a = np.max(separation_distances[:frame+1])  # Maximum separation distance up to the current frame
    r_p = np.min(separation_distances[:frame+1])  # Minimum separation distance up to the current frame
